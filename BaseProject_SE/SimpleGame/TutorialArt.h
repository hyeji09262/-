...
inline void Person(Vec p, Color cloak, bool wizard = false, int variant = 0)
{
    ...
    if (wizard)
    {
        ...
        Tri({s.x - 10, s.y - 51 + breath}, {s.x - 3, s.y - 77 + breath},
            {s.x + 9, s.y - 51 + breath}, cloak);
        Tri({s.x - 3, s.y - 77 + breath}, {s.x + 9, s.y - 51 + breath},
            {s.x + 4, s.y - 54 + breath}, Tint(cloak, 1.3f));
        ...
        Diamond(orb, 5, 8, teal);
        Diamond(orb, 2, 4, {.92f, 1, 1, 1});
    }
    else
    {
        if (variant % 3 == 0)
        {
            Ellipse({s.x, s.y - 52}, 11, 4, dark);
            Box(s.x - 7, s.y - 59, 14, 7, cloak);
        }
        if (variant % 3 == 1)
            Quad({s.x - 7, s.y - 47}, {s.x + 6, s.y - 47}, {s.x + 5, s.y - 39}, {s.x - 8, s.y - 40},
                 {.8f, .74f, .64f, 1});
        if (variant % 3 == 2)
            Box(s.x - 6, s.y - 32, 12, 16, {.69f, .62f, .5f, 1});
    }
}
...
{
    Vec s = Project(p.p);
    int variant = p.kind - 1;
    Color wall = variant == 1 ? Color{.44f, .45f, .44f, 1} : Color{.63f, .51f, .38f, 1};
    Color roof = variant == 2   ? Color{.51f, .40f, .23f, 1}
                 : variant == 1 ? Color{.26f, .32f, .38f, 1}
                                : Color{.43f, .24f, .23f, 1};
    ...
    for (int row = 0; row < 4; ++row)
    {
        ...
        if (variant == 1)
            for (int k = 0; k < 3; ++k)
            {
                ...
                Line({x, y - (x - s.x) * .48f}, {x, y - 13 - (x - s.x) * .48f}, 1,
                     Tint(wall, .63f));
            }
    }
    if (variant != 1)
    ...
    Quad({s.x - 5, s.y - 61}, {s.x - 56, s.y - 84}, {s.x - 7, s.y - 122}, {s.x + 45, s.y - 99},
         Tint(roof, .7f));
    Quad({s.x - 5, s.y - 61}, {s.x + 57, s.y - 86}, {s.x + 45, s.y - 99}, {s.x - 7, s.y - 122},
         roof);
    for (int row = 1; row <= 5; ++row)
    {
        ...
        if (variant == 2)
            ...
            {
                float u = k / 8.f;
                Vec at = {a.x + (b.x - a.x) * u, a.y + (b.y - a.y) * u};
    ...
    Quad({s.x - 33, s.y - 42}, {s.x - 14, s.y - 33}, {s.x - 14, s.y - 53}, {s.x - 33, s.y - 62},
         gold);
    Line({s.x - 24, s.y - 58}, {s.x - 24, s.y - 38}, 2, ink);
    Line({s.x - 33, s.y - 52}, {s.x - 14, s.y - 43}, 2, ink);
    Glow({s.x - 23, s.y - 46}, 28, 28, {1, .57f, .22f, .25f});
    Box(s.x + 19, s.y - 119, 12, 24, {.37f, .35f, .36f, 1});
    Box(s.x + 17, s.y - 121, 16, 5, {.24f, .24f, .27f, 1});
    for (int i = 0; i < 3; ++i)
    {
        float rise = std::fmod(clockTime * 13 + i * 18, 54.f);
        Ellipse({s.x + 25 + std::sin(rise * .06f) * 7, s.y - 124 - rise}, 5 + rise * .13f,
                4 + rise * .08f, {.53f, .50f, .56f, (1 - rise / 54) * .13f}, 12);
    }
}
...
inline void DrawProp(const Prop& p)
{
    ...
    if (p.kind == 0)
    {
        ...
        Box(s.x - 4, s.y - 46, 8, 46, {.25f, .20f, .17f, 1});
        Box(s.x - 2, s.y - 44, 2, 43, {.40f, .30f, .20f, 1});
        for (int layer = 0; layer < 3; ++layer)
        {
            float y = s.y - 21 - layer * 24, w = 34 - layer * 6;
            Color c = {.12f + layer * .035f, .28f + layer * .027f, .25f + layer * .025f, 1};
            Tri({s.x - w + sway, s.y - 18 - layer * 24}, {s.x + sway, y - 58}, {s.x + w + sway, y},
                c);
            Tri({s.x + sway, y - 58}, {s.x + w + sway, y}, {s.x + 3 + sway, y - 5}, Tint(c, .65f));
            Line({s.x - w * .6f + sway, y - 11}, {s.x - 2 + sway, y - 43}, 1,
                 {.30f, .40f, .30f, .6f});
        }
    }
    else if (p.kind == 4)
    ...
    {
        Quad({s.x - 16, s.y - 5}, {s.x - 9, s.y - 23}, {s.x + 9, s.y - 20}, {s.x + 18, s.y - 3},
             {.38f, .42f, .44f, 1});
...
inline void DrawWorld()
{
    ...
    for (int sum = 0; sum < MapWidth + MapHeight; ++sum)
    ...
    for (int i = 0; i < int(props.size()); ++i)
        if (Visible(Project(props[i].p)))
            items.push_back({props[i].p.x + props[i].p.y, 0, i});
    for (int i = 0; i < NPCCount; ++i)
        items.push_back({villagers[i].p.x + villagers[i].p.y, 1, i});
    items.push_back({player.x + player.y, 2, 0});
    if (stage < 2)
        items.push_back({boss.x + boss.y, 3, 0});
    for (int i = 0; i < int(monsters.size()); ++i)
        if (monsters[i].hp > 0)
            items.push_back({monsters[i].p.x + monsters[i].p.y, 4, i});
    ...
    {
        if (item.type == 0)
            DrawProp(props[item.index]);
        if (item.type == 1)
            Person(villagers[item.index].p,
                   {.38f + (item.index % 4) * .09f, .34f + (item.index % 3) * .07f, .40f, 1}, false,
                   item.index);
        if (item.type == 2)
        ...
        if (item.type == 3)
            BossArt();
        if (item.type == 4)
            MonsterArt(monsters[item.index]);
        if (item.type == 5)
        {
    ...
    if (beam > 0)
    {
        ...
        Line(a, b, 12, {.25f, .70f, 1, .12f});
        Line(a, b, 5, {.35f, .93f, 1, .55f});
