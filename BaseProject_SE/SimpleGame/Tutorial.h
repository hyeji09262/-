...
{
    renderer->Text(x * width / 1280, y * height / 800, text, size * height / 800, c.r, c.g, c.b,
                   c.a);
}
...
inline void Interact()
{
    ...
    if (Dist(player, shrine) < (upgrade == 1 ? 4.f : 2.1f))
    {
        if (stage == 3)
        {
            ...
            Say("그림자 봉인이 풀렸습니다! 새벽 불씨를 되찾았어요. 촌장에게 돌아가세요.");
        }
        else if (stage < 3)
            Say("그림자 봉인입니다. 숲의 파수꾼을 물리치고 마법을 먼저 배워야 합니다.");
        else
            Say("새벽 불씨는 주머니에 안전하게 있습니다. 촌장에게 돌아가세요.");
        return;
    }
    int i = NearestNPC();
    if (i < 0)
    ...
    else if (i == 0 && stage == 4)
    {
        stage = 5;
        finishTime = elapsed;
        Say("모라: 빛이 돌아왔군요! 세금도 돌아왔지만요. 고마워요, 마법사님.");
    }
    else if (i == 0 && stage == 3)
        Say("동쪽 해안 제단에 그림자 마법을 써서 새벽 불씨를 찾아주세요.");
    else
    ...
    if (keys[' '] && attackCD <= 0)
    {
        ...
        if (hitBoss)
        {
            if (--bossHP <= 0)
            {
                ...
                Say("파수꾼을 물리쳤습니다! 마법 핵심과 스킬 포인트를 얻었습니다.");
            }
        }
...
inline void Draw()
{
    ...
    const char* objectives[] = {
        "01  촌장 모라에게 말 걸기 [E]",       "02  숲의 파수꾼 처치하기 [스페이스]",
    ...
    if (stage == 1 && Dist(player, boss) < 6.5f)
    {
        ...
        Text(430, 196,
             pulse > 2.1f ? "파수꾼: 공격 준비! 붉은 범위 밖으로 피하세요!"
                          : "숲의 파수꾼 · 스페이스를 눌러 마법 공격",
             gold, 16);
    }
